...
class FeatureLoss(nn.Module):
    ...
    def forward(self,
        ...
        mask=self.resize_mask(mask[...,0].type(torch.float64),preds_S.shape)
        ...
        Mask_fg = mask.squeeze(1)/255#torch.zeros_like(S_attention_t)
        Mask_bg = 1-Mask_fg
        Mask_fg=Mask_fg/Mask_fg.sum(dim=(-1,-2)).reshape(-1,1,1)
        Mask_bg=Mask_bg/Mask_bg.sum(dim=(-1,-2)).reshape(-1,1,1)
        wmin,wmax,hmin,hmax = [],[],[],[]
        # for i in range(N):
